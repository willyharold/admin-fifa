...
export class MatiereService {
  ...
  constructor(
    ...
    this.url = config.getUrl() + "/matiere";
  }
  ...
  getAll(): Promise<Matiere[]> {
    return this.http.get(this.url, this.config.jwt())
      .toPromise()
      .then(response => response.json() as Matiere[])
      .catch(this.handleError);
  }

  getById(id: number): Promise<Matiere> {
  ...
  getUploadUrl(data: Matiere): string {
  ...
  findByClasseAnnee(classeId, anneeId): Promise<Matiere[]> {
